...
public class AiDamageProjectile : MonoBehaviour
{
    ...
    public void Shoot(Vector3 origin, Vector3 mid, Vector3 end, float time, float dmg, Transform castOrigin, int penetrations)
    {
        ...
        visualiser.Start(castOrigin, transform);
        maxPenetrations = penetrations;
        healths.Clear();
        this.penetrations = 0;
    }

    public void Shoot(Vector3 origin, Vector3 mid, Transform end, Vector3 offset, float time, float dmg, Transform castOrigin, int penetrations)
    {
        Debug.Log("AAA");
        damage = dmg;
        transform.position = origin;
